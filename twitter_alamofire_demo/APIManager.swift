...
class APIManager: SessionManager {
    ...
    static let consumerKey = "fdGpcfhELU91AbyNkQO5yJQaf"
    static let consumerSecret = "uM8vV8xFlwSeEyIPMrW92JJP2gNGeewixucwIxsWkISZUtILx3"
        ...
        oauthManager.authorize(withCallbackURL: callbackURL, success: { (credential, _response, parameters) in
            ...
            self.getCurrentAccount(completion: { (user, error) in
                if let error = error {
                    ...
                    let loggedInUser: [String: Any] = ["name": user.name, "screen_name": user.screenName!, "profile_image_url_https": user.profileImage!, "description": user.saying, "friends_count": user.followingCount, "profile_background_image_url_https": user.backgroundImageURL, "statuses_count": user.tweetsCount, "followers_count": user.followersCount]
                    // MARK: TODO: set User.current, so that it's persisted
                    user.dictionary = loggedInUser
                    User.current = user
                    success()
                    success()
                }
...
//        }
    ...
    func getMoreTimeLine(_ tweet: Tweet, completion: @escaping ([Tweet]?, Error?) -> ()) {
        let urlString = "https://api.twitter.com/1.1/statuses/home_timeline.json"
        let parameters = ["max_id": tweet.idString]
